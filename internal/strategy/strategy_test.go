...
func TestEvaluate(t *testing.T) {
	tests := []struct {
		...
		{
			name:       "yes signal: yesAsk=60, buy YES at ask-1=59",
			yesBid:     55,
			yesAsk:     60,
			wantSide:   "yes",
			wantLimit:  59,
		...
		{
			name:       "yes signal: yesAsk=55 (exact threshold)",
			yesBid:     50,
			yesAsk:     55,
			wantSide:   "yes",
			wantLimit:  54,
		...
		{
			name:       "no signal: noAsk=60 (yesBid=40, yesAsk=45)",
			yesBid:     40,
			yesAsk:     45,
			wantSide:   "no",
			wantLimit:  59,
		...
		{
			name:       "no signal: noAsk=55 (yesBid=45, yesAsk=50)",
			yesBid:     45,
			yesAsk:     50,
			wantSide:   "no",
			wantLimit:  54,
		...
		{
			name:       "yes signal takes priority: yesAsk=70, noAsk=70",
			yesBid:     30,
			yesAsk:     70,
			wantSide:   "yes",
			wantLimit:  69,
